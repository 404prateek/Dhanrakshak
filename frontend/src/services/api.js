...
export const api = {
  ...
  // Documents — accepts optional onUploadProgress callback for progress UI
  uploadDocument: async (caseId, file, onUploadProgress, signal) => {
    ...
    const response = await fetch(`${API_BASE_URL}/cases/${caseId}/documents`, {
      ...
      signal,
      headers: {
        Authorization: `Bearer ${localStorage.getItem('dhanrakshak_token') || ''}`,
      },
    });
...
export const runMLAnalysis = async (caseId, filePaths, behaviorData = {}) => {
  ...
  try {
    ...
    clearTimeout(timeoutId);
    throw error; // Re-throw — let the UI handle and display the real error
  }
};
...
export const runCrossDocAnalysis = async (caseId, primaryPath, secondaryPath) => {
  ...
  try {
    ...
    if (!response.ok) {
      const err = await response.text();
      throw new Error(`Cross-doc analysis failed: ${response.status} - ${err}`);
    }
    return await response.json();
  } catch (error) {
    clearTimeout(timeoutId);
