...
DEFAULT_TIMEOUT_SECONDS = 30  # fast fallback to template if Ollama is slow
...
class OllamaReporter:
    ...
    def _template_report(context: Dict[str, Any]) -> str:
        ...
        report = (
            f"DhanRakshak Fraud Investigation Report\n"
            f"======================================\n"
            f"Audit ID   : {audit_id}\n"
        ...
        verdict_map = {
            "HIGH":   "Application presents strong indicators of fraud. Immediate escalation to risk team required.",
            "MEDIUM": "Application shows anomalies requiring manual verification before proceeding.",
            "LOW":    "Application appears consistent. Proceed with standard due-diligence checks.",
        }
        report += verdict_map.get(risk_level, "Risk level undetermined. Manual review recommended.") + "\n"
        report += "\n(Generated by template engine — Ollama offline)\n"
        return report
